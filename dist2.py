...
def PDC(x1,x2):
    ...
    return (delta_m*(1/5) + delta_p*(3/5) + delta_v*(1/5))
...
def phonetic_difference(x1, x2):
    ...
    if raw_difference <= 0.5:
        return (2*(raw_difference ** 2))*1.25
    else:
        return (1 - (1 - raw_difference) ** 2)*1.25
...
def find_minimal_pair_from_file(input_file, file1, file2, output_file):
    """
    Finds the minimal match for a list of words provided in an input file 
    in file1 (based on their transcription and writing) and file2, 
    and writes the results to an output file.
    """
    # Read the input words from the input file
    with open(input_file, 'r', encoding='utf-8') as infile:
        word_list = [line.strip() for line in infile if line.strip()]
    
    # Read the first file into a dictionary for quick lookup
    with open(file1, 'r', encoding='utf-8') as f1:
        words1_dict = {line.split(' ', 1)[0]: line.split(' ', 1)[1].strip() for line in f1 if line.strip()}
    ...
    with open(output_file, 'w', encoding='utf-8') as out:
        for word in word_list:
            if word not in words1_dict:
                out.write(f"{word}\tNOT_FOUND_IN_FILE1\n")
                continue
            
            ipa1 = words1_dict[word]
            min_distance = float('inf')
            best_match = None
            
            for word2, ipa2 in words2:
                # Compute Levenshtein distance between words
                word_distance = Levenshtein.distance(word, word2)
            ...
            # Write the best match for the current word to the output file
            if best_match:
                out.write(f"{word}\t{ipa1}\t{best_match[0]}\t{best_match[1]}\t{best_match[2]}\n")

# Example usage
input_file = "input_words.txt"  # Input file with a list of words, one per line
file1 = "./nouns/central_cat.txt"  # First file with words and transcriptions
file2 = "./nouns/italian.txt"      # Second file with words and transcriptions
output_file = "minimal_matches_from_input_file.txt"  # Output file to store results

find_minimal_pair_from_file(input_file, file1, file2, output_file)
